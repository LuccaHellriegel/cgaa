...
export class CampState {
	...
	constructor(
		...
		const sideArrowPoints = ArrowHeadPolygon.points(x - 2 * halfSize - 10, y, 1.5 * halfSize, 2 * halfSize).map(
			(point) => Phaser.Math.RotateAround(point, x - 2 * halfSize - 10, y, Phaser.Math.DegToRad(-90))
		);
		...
		const crossPoints = SymmetricCrossPolygon.points(x - 2 * halfSize - 10, y, 2 * halfSize, 0.4 * halfSize).map(
			(point) => Phaser.Math.RotateAround(point, x - 2 * halfSize - 10, y, Phaser.Math.DegToRad(45))
		);
		//after rotating the arrow, height and width are reversed
