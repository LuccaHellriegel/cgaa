...
import { InteractionSelectBar } from "../ui/select/bars/InteractionSelectBar";
import { Popup } from "../ui/Popup";
import { ClickableImageRect, ImageRect } from "../ui/rect/DoubleRect";
import { FriendCounter } from "../ui/counters/FriendCounter";
...
export class HUD extends Phaser.Scene {
  ...
  private setupEventListeners() {
    ...
    this.ourGame.events.on(
      EventSetup.healPlayer,
      this.playerHealthBar.increase.bind(this.playerHealthBar)
    );
  ...
  create() {
    var config = {
      key: "game-over-anim",
      frames: this.anims.generateFrameNumbers("game-over", {
        start: 0,
        end: 30,
      }),
      frameRate: 10,
    ...
    let healerSelectBar = new TowerSelectBar(
      this,
      0 + 180,
      0 + 30 + 5 + 5,
      "healer"
    );
    let shooterSelectBar = new TowerSelectBar(
      this,
      0 + 180,
      0 + 30 + 5 + 5,
      "shooter"
    ...
    let interactionSelectBar = new InteractionSelectBar(
      ...
      this.ourGame.cgaa.questManager,
      selectionManager
    );
    ...
    (
      interactionSelectBar.contentElements[0] as ClickableImageRect
    ).setInteractive("pointerdown", questFunc);
    ...
    let sellFunc = function () {
      if (selectionManager.selectedUnit) {
        ...
        this.ourGame.events.emit(
          EventSetup.towerSoldEvent,
          selectionManager.selectedUnit.type
        );
        selectBars.forEach((bar) => bar.hide());
        this.ourGame.cgaa.input.selectorRect.turnOff();
      }
    }.bind(this);
    (healerSelectBar.contentElements[0] as ClickableImageRect).setInteractive(
      "pointerdown",
      sellFunc
    );
    (shooterSelectBar.contentElements[0] as ClickableImageRect).setInteractive(
      "pointerdown",
      sellFunc
    );

    selectionManager.setSelectBars(
      healerSelectBar,
      shooterSelectBar,
      interactionSelectBar
    );
    selectBars.forEach((bar) => bar.hide());

    let shooterCounter = new PureCounter(
      this,
      0,
      0,
      "",
    ...
    let healerCounter = new PureCounter(
      this,
      0,
      0,
      "",
    ...
    (buildBar.contentElements[0] as ClickableImageRect).setInteractive(
      ...
      function () {
        ...
        if ((buildBar.contentElements[0] as ImageRect).selected) {
          ...
          (
            this.ourGame.cgaa.input.build as BuildManager
