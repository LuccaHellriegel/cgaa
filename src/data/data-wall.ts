...
function splitRelPosArrAtGaps(positions: RelPos[]): RelPos[][] {
  const tester = (lastPos, curPos) => {
    ...
    const distanceInOneDirectionDetected =
      (Math.abs(lastPos.column - curPos.column) > 1 &&
        Math.abs(lastPos.row - curPos.row) == 0) ||
      (Math.abs(lastPos.row - curPos.row) > 1 &&
        Math.abs(lastPos.column - curPos.column) == 0);
...
export function addWallSidesToGameMap(
  gameMap: GameMap,
  wallSides: WallSide[],
  wallSymbol
) {
