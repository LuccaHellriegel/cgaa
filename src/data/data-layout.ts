...
function areaLayoutToMapDimensions(areaLayout: any[][], areaSize: number) {
  return {
    xAxis: areaLayout[0].length * areaSize,
    yAxis: areaLayout.length * areaSize,
  };
}

export function areaLayoutToMap(
  areaLayout: any[][],
  areaSize: number,
  defaultValue: number
) {
...
export function areaLayoutToWallSides(
  areaLayout: (0 | 1)[][],
  areaSize: number
): WallSide[] {
  const areaSymbol = 1;
...
function layoutExitToExitPositions(
  areaTopLeft: RelPos,
  exitSide: ExitSide,
  areaSize: number
) {
...
export function exitLayoutToExits(
  exitLayout: ExitSide[][][],
  areaSize: number
): Exit[] {
  let result: Exit[] = [];
  ...
  for (let row = 0; row < exitLayout.length; row++) {
    for (let column = 0; column < exitLayout[0].length; column++) {
      ...
      const areaTopLeft = layoutAreaToMapTopLeft(
      ...
      for (const exitSide of exitLayout[row][column]) {
        if (exitSide.side !== "none") {
          const positionsInMap = layoutExitToExitPositions(
            areaTopLeft,
            exitSide,
            areaSize
          );
...
export function mapAreaTopLeftToMapMiddle(
  topLeftInRelationToMap: RelPos,
  areaSize: number
) {
...
export function filterRelPosInsideArea(
  areaMapTopLeft: RelPos,
  areaSize: number,
  pos: RelPos[]
) {
...
export function mapSpawnablePos(map: GameMap, mapDefaultValue, exits: Exit[]) {
  ...
  for (const exit of exits) {
    for (const pos of exit.positionsInMap) {
      posAroundExit = posAroundExit.concat(
        posAround2DPosition(pos.row, pos.column, forbiddenShape)
      );
    }
  }
...
export function randomMapCampIDsToAreas(
  campIDs: string[],
  areaLayout: any[][],
  areaSymbol
): CampArea[] {
  ...
  static exitSides: ExitSide[][][] = [
    [
      ...
      [
        {
          side: "down",
          width: EnvSetup.exitWidth,
          middlePosition: EnvSetup.areaExit,
        },
      ],
      [
        {
