...
export function posAround2DPosition(startRow, startColumn, posShape) {
	...
	const [row, column] = array2DFind(posShape, (posSymbol) => posSymbol === startPosSymbol);
...
export function posAround2DPositionRelPos(startRow, startColumn, posShape) {
	return posAround2DPosition(startRow, startColumn, posShape).map((pos) => new RelPos(pos.row, pos.column));
}
