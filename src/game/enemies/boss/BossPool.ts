...
import { EnemyConfig, EnemySize, WeaponTypes, EnemyFactory } from "../unit/EnemyFactory";
	...
	private initPool() {
		for (let index = 0; index < this.numberOfGroups; index++) {
			for (let index = 0, length = this.groupComposition.length; index < length; index++) {
				...
				let enemy: EnemyCircle = EnemyFactory.createBoss(this.enemyConfig, this.enemies);
				enemy.poolDestroy();
