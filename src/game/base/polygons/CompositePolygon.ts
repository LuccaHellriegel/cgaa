...
import { ArrowHeadPolygon } from "./ArrowHeadPolygon";
...
export class CompositePolygon {
  ...
  constructor(config) {
    ...
    config.forEach(polygonConfig => {
      if (polygonConfig[4] == "rect") {
        ...
        this.polygons.push(
          new ArrowHeadPolygon(polygonConfig[0], polygonConfig[1], polygonConfig[2], polygonConfig[3])
        );
      } else {
  ...
  private calculateHeight() {
    ...
    this.polygons.forEach(polygon => {
      let { lowestY, highestY } = polygon.getLowestHighestY();
  ...
  private calculateWidth() {
    ...
    this.polygons.forEach(polygon => {
      let { lowestX, highestX } = polygon.getLowestHighestX();
  ...
  checkForCollision(other) {
    for (let index = 0; index < this.polygons.length; index++) {
      ...
      if (currentPolygon.type === "polygon" && other.type === "polygon") {
          
      } else if (currentPolygon.type === "circle" && other.type === "circle") {
        collided = (currentPolygon as CirclePolygon).checkForCollisonWithOtherCircle(other);
      } else {
        collided = collision(currentPolygon, other);
      }
