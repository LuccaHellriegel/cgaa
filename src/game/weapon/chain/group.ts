...
export class ChainWeapons extends Phaser.Physics.Arcade.Group {
	constructor(scene, private ownerSizeName, amount, public weaponGroup: Phaser.Physics.Arcade.Group) {
	...
	placeWeapon(x, y) {
		...
		if (weapon.initialized) {
			...
			weapon.init(this.ownerSizeName, x, y, unitAmountConfig[this.ownerSizeName].amount, this.weaponGroup);
		}
		return weapon;
	}
}
