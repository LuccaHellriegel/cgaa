...
import {
  ...
  WeaponChain,
  WeaponGeoms,
} from "../data/chainWeaponBase";
import {
  weaponTopLefts,
  weaponHeights,
  weaponRadius,
  weaponDists,
} from "../data/chainWeaponData";
// GEOM
function weaponGeomsPerSize() {
  ...
  for (let unitSize of cirlceSizeNames) {
    ...
    result[unitSize] = topDownWeaponGeom(
      topLefts,
      arrowConfig,
      weaponChainConfig,
      unitSize
...
function topDownWeaponGeom(
  ...
  return {
    ...
    frame1: topDownWeaponGeomFrame1(
      topLefts.frame1,
      arrowConfig,
      weaponChainConfig,
      unitSize
    ),
    frame2: topDownWeaponGeomFrame2(
      topLefts.frame2,
      arrowConfig,
      weaponChainConfig,
      unitSize
...
function topDownWeaponGeomFrame0(
  topLeft: Point,
  arrowConfig: ArrowConfig,
  unitSize: EnemySize
) {
...
function topDownWeaponGeomFrame1(
  ...
  let bigChainTopCenter = {
    x: arrowCenterX,
    y: arrowCenterY + distArrowAndChain,
  };
  // calculate bigChain
  let bigChain = {
    radius: bigRadius,
    points: topDownChain(bigChainTopCenter, distBetweenBigCircles, bigCircles),
  };
...
function topDownWeaponGeomFrame2(
  ...
  let smallChainTopCenter = {
    x: arrowCenterX,
    y: lowestJoint.y + distBetweenBigAndSmallChain,
  };
