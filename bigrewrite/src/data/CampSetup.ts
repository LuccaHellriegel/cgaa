export class CampSetup {
  ...
  static campBitMasks: number[] = CampSetup.campIDs.map(
    (_, index) => 1 << index
  );
  ...
  static specialCampMasks: number[] = [
    CampSetup.playerCampMask,
    CampSetup.bossCampMask,
  ];
  static ordinaryCampIDs: CampID[] = CampSetup.campIDs.filter(
    (id) => !CampSetup.specialCampIDs.includes(id)
  );
  ...
  static colorDict: { [key in CampID]: number } = {
    ...
    purple: 0x7f00ff,
    blue: 0x6495ed,
    boss: null as unknown as number,
